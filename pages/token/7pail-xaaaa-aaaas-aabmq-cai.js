...
const TokenPage = () => {
  const tokenId = "7pail-xaaaa-aaaas-aabmq-cai"; // Get tab from the query
  ...
  const pageTitle = "Bob Price Chart | ICP Coin market cap";

  return (
    <>
      <Head>
        <title>{pageTitle}</title>
        <meta name="description" content={`Real-time BOB token price updates. BOB/ICP is the leading trading pair on ICPSwap exchange - track live price and volume data.`} />
        <meta property="og:title" content={`${ pageTitle }`} />
        <meta property="og:description" content={`Real-time BOB token price updates. BOB/ICP is the leading trading pair on ICPSwap exchange - track live price and volume data.`} />          
